...
import type { Raster, PixelArtOptions, PixelateResult } from "../domain/pixelArt.js";
...
type AvatarLabStatus = (typeof AVATAR_LAB_STATUS)[keyof typeof AVATAR_LAB_STATUS];
...
// correr el pipeline puro de pixelArt.ts (~5ms) sobre ese raster ya
// recortado, en vez de repetir la segmentación en cada cambio.
export function useAvatarLab() {
  const [status, setStatus] = useState<AvatarLabStatus>(AVATAR_LAB_STATUS.IDLE);
  const [params, setParams] = useState<PixelArtOptions>(DEFAULT_OPTIONS);
  const [result, setResult] = useState<PixelateResult | null>(null);
  const [error, setError] = useState<string | null>(null);
  const cutoutRasterRef = useRef<Raster | null>(null);

  const runPixelate = useCallback((raster: Raster, currentParams: PixelArtOptions) => {
