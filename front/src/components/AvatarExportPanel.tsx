...
function rasterToCanvas(raster: Raster): HTMLCanvasElement {
  ...
  // Un canvas recién creado siempre puede dar su contexto 2D en cualquier
  // browser real; null solo pasaría en un entorno sin soporte de canvas.
...
// Baja el PNG y arma el snippet para pegar en avatars.ts. El pegado a
// avatars.ts queda manual a propósito (no escribe en el repo) — mismo flujo
// que se usó para sumar los 48 avatares actuales, solo que ahora el sprite
// se generó adentro en vez de conseguirse afuera.
export function AvatarExportPanel({ raster, disabled = false }: AvatarExportPanelProps) {
  const nameId = useId();
  ...
  return (
    <div className="avatar-export-panel">
      ...
      {snippet && (
        <div className="field">
          <label>Para pegar en avatars.ts</label>
