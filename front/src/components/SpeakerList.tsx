...
import type { Participant, SpeakerTimerState } from "../domain/types.js";
...
interface SpeakerListProps {
  ...
  isHost: boolean;
  onSetSpeaker: (participantId: string) => void;
  onClearSpeaker: () => void;
}
...
// en cliente.
export function SpeakerList({ participants, currentSpeakerId, speakerTimer, isHost, onSetSpeaker, onClearSpeaker }: SpeakerListProps) {
  if (!participants || participants.length === 0) {
    return <p className="field-help">Todavía no hay nadie en la sala.</p>;
  }

  function handleClick(participantId: string) {
    if (!isHost) return;
