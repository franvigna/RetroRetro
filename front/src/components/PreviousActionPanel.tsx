...
// calcula igual que en el backend (ver domain/previousAction.ts): separar por
...
export function previousActionLines(notes: string | null | undefined): string[] {
  return (notes || "").split("\n").filter((line) => line.trim().length > 0);
}

interface PreviousActionPanelProps {
  notes: string | null | undefined;
  checks?: Record<number, boolean>;
  onSetItem?: (index: number, done: boolean | null) => void;
  canToggle?: boolean;
}

export function PreviousActionPanel({ notes, checks = {}, onSetItem, canToggle = false }: PreviousActionPanelProps) {
