...
export function buildActionPlanFilename(room: Room, date = new Date()): string {
...
function fillPageBackground(doc: jsPDF, pageWidth: number, pageHeight: number): void {
  doc.setFillColor(...ARCADE_COLORS.bg);
  doc.rect(0, 0, pageWidth, pageHeight, "F");
}

function drawBezelStripe(doc: jsPDF, pageWidth: number): void {
...
function drawAvatar(doc: jsPDF, avatarId: string | null | undefined, x: number, y: number, size = 18): boolean {
...
export function generateActionPlanPdf(room: Room, { now = new Date() }: { now?: Date } = {}): jsPDF {
  ...
  const participantsById: Record<string, Participant> = Object.fromEntries(room.participants.map((p) => [p.id, p]));
  ...
  for (const card of actionCards) {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    const textLines: string[] = doc.splitTextToSize(card.text, textWidth);
