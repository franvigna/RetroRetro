...
// ventaja se pierde igual a la resolución final de pixelArt.ts).
//
...
// tocar solo este archivo, no el resto del pipeline en pixelArt.ts.
import type { ImageSegmenter } from "@mediapipe/tasks-vision";
import type { Raster } from "./pixelArt.js";
...
export function loadSegmenter(): Promise<ImageSegmenter> {
  if (!segmenterPromise) {
    segmenterPromise = (async () => {
      const { FilesetResolver, ImageSegmenter: ImageSegmenterImpl } = await import("@mediapipe/tasks-vision");
      const vision = await FilesetResolver.forVisionTasks(WASM_BASE_URL);
      return ImageSegmenterImpl.createFromOptions(vision, {
...
function drawBitmapToRaster(bitmap: ImageBitmap, maxDimension: number): { canvas: OffscreenCanvas; raster: Raster } {
...
// de "persona" de MediaPipe (0-255) — listo para pixelArt.ts.
export async function cutoutForeground(bitmap: ImageBitmap): Promise<Raster> {
  ...
  try {
    ...
    const out: Raster = { width: raster.width, height: raster.height, data: Uint8ClampedArray.from(raster.data) };
