...
import type { RGB } from "./arcadePalette.js";
...
export function findContentBounds(raster: Raster, alphaThreshold = DEFAULT_OPTIONS.alphaThreshold): Bounds | null {
...
// contenido).
export function squareCrop(raster: Raster, bounds: Bounds, paddingRatio = DEFAULT_OPTIONS.paddingRatio): Raster {
...
// desaparece.
export function downsampleToGrid(raster: Raster, size: number = GRID_SIZE): Raster {
  const { width: srcW, height: srcH, data: srcData } = raster;
...
export function hardenAlpha(raster: Raster, alphaThreshold = DEFAULT_OPTIONS.alphaThreshold): Raster {
  const cutoff = alphaThreshold * 255;
...
export function adjustColors(raster: Raster, { saturation = 1, contrast = 1 }: { saturation?: number; contrast?: number } = {}): Raster {
  const out = { width: raster.width, height: raster.height, data: Uint8ClampedArray.from(raster.data) };
  const { data } = out;
...
export function buildPalette(raster: Raster, colorCount: number): RGB[] {
  const { data } = raster;
  const pixels: RGB[] = [];
  for (let i = 0; i < data.length; i += 4) {
...
export function applyPalette(raster: Raster, palette: RGB[]): Raster {
  const out = { width: raster.width, height: raster.height, data: Uint8ClampedArray.from(raster.data) };
...
export function snapPaletteToTheme(palette: RGB[], themeRgb: RGB[], maxDistance = 40): RGB[] {
...
export function pixelate(raster: Raster, options: PixelArtOptions = {}): PixelateResult | null {
