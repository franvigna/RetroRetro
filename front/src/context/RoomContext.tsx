...
import { socket, type AppSocket } from "../socket/client.js";
import type { RoomCreatePayload } from "../socket/events.js";
import type { Room } from "../domain/types.js";

export type ConnectionStatus = "connecting" | "connected" | "disconnected" | "server_full";
...
interface LastError {
  type: "unauthorized" | "invalid_action" | "rate_limited";
  action: string;
  reason?: string;
}
...
interface RoomContextValue {
  ...
  roomLockedCode: string | null;
  roomClosed: boolean;
  pendingRejoin: boolean;
  pendingRejoinName: string | null;
  createRoom: (payload: RoomCreatePayload) => void;
  joinRoom: (code: string, name: string, avatarId?: string | null) => void;
  leaveRoom: () => void;
  clearError: () => void;
  clearRoomNotFound: () => void;
  clearRoomLocked: () => void;
...
function storeIdentity(identity: Identity): void {
  try {
...
export function RoomProvider({ children }: { children: ReactNode }) {
  ...
  const [roomLockedCode, setRoomLockedCode] = useState<string | null>(null);
  ...
  const identityRef = useRef<Identity>(loadStoredIdentity());
  ...
  useEffect(() => {
    function handleConnect() {
      ...
      // (ver room:join en back/src/socket/handlers/roomHandlers.ts).
    ...
    function handleConnectError(err: Error) {
      ...
      // capacidad (ver back/src/socket/index.ts, MAX_CONCURRENT_CONNECTIONS)
    ...
    function handleRoomCreated({ room: createdRoom, sessionToken }: { code: string; room: Room; sessionToken: string }) {
      setRoom(createdRoom);
      setCurrentParticipantId(createdRoom.hostId);
      // sessionToken es la credencial privada de reconexión del host — nunca
      // viaja en `room` (ver back/src/domain/room.ts, toPublicRoom), solo acá.
    ...
    function handleRoomJoined({ participantId, sessionToken }: { participantId: string; sessionToken: string }) {
      setCurrentParticipantId(participantId);
      identityRef.current = { ...identityRef.current, sessionToken };
      storeIdentity(identityRef.current);
    }

    function handleRoomState({ room: nextRoom }: { room: Room }) {
      setRoom(nextRoom);
      setRoomNotFoundCode(null);
      setRoomLockedCode(null);
    }

    function handleRoomNotFound({ code }: { code: string }) {
    ...
    // back/src/socket/handlers/roomHandlers.ts e isRoomLockedForNewJoins.
    function handleRoomJoinLocked({ code }: { code: string }) {
    ...
    function handleTimerTick({ remainingSeconds }: { remainingSeconds: number }) {
    ...
    function handleSpeakerTick({ remainingSeconds }: { remainingSeconds: number }) {
      setRoom((prev) =>
        prev?.speakerTimer ? { ...prev, speakerTimer: { ...prev.speakerTimer, remainingSeconds } } : prev
      );
    }

    function handleUnauthorized(payload: { action: string }) {
      setLastError({ type: "unauthorized", ...payload });
    }

    function handleInvalidAction(payload: { action: string; reason: string }) {
      setLastError({ type: "invalid_action", ...payload });
    }

    // El servidor frena eventos repetidos en muy poco tiempo (spam de
    // tarjetas/votos/joins) — ver back/src/socket/rateLimiter.ts. En uso
    // normal esto no debería dispararse nunca.
    function handleRateLimited(payload: { action: string }) {
  ...
  const joinRoom = useCallback((code: string, name: string, avatarId: string | null = null) => {
