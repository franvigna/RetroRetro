...
function keyFor(socketId: string, action: string): string {
...
export function isRateLimited(socketId: string, action: string, { max, windowMs }: RateLimitConfig): boolean {
...
// entradas de sockets que ya no existen (ver socket/index.ts, "disconnect").
