...
function applyPhaseTransition(
  socket: TypedSocket,
  action: string,
  transitionFn: (room: Room) => Room,
  { broadcastRoomState, emitError, startRoomTimerIfNeeded }: HandlerContext
): void {
...
export function registerPhaseHandlers(io: TypedServer, socket: TypedSocket, ctx: HandlerContext): void {
  ...
  // ✓ y ✕, en vez de un único toggle) — ver domain/previousAction.ts.
