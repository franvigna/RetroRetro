...
function applyTimerChange(socket, action, transitionFn, { broadcastRoomState, emitError, startRoomTimerIfNeeded, startSpeakerTimerIfNeeded }) {
  ...
  try {
    ...
    if (room.phase === "expression_round") {
      ...
      const nextSpeakerTimer = transitionFn(room.speakerTimer);
    ...
    const nextTimer = transitionFn(room.timer);
...
export function registerTimerHandlers(io, socket, ctx) {
  ...
  socket.on("timer:add_time", ({ seconds } = {}) => {
    applyTimerChange(socket, "timer:add_time", (timer) => addTime(timer, seconds), ctx);
  });
}
