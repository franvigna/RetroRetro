...
import type { ClientToServerEvents, ServerToClientEvents, SocketData, TypedServer, TypedSocket } from "./events.js";
import type { Room, CardColumn } from "../domain/types.js";
...
export function setupSocket(
  ...
  { maxConnections = DEFAULT_MAX_CONCURRENT_CONNECTIONS }: SetupSocketOptions = {}
): TypedServer {
  const io: TypedServer = new Server<ClientToServerEvents, ServerToClientEvents, Record<string, never>, SocketData>(
    httpServer,
  ...
  const KEEP_IMPROVE_TRY_COLUMNS: CardColumn[] = ["keep", "improve", "try"];

  function roomStateFor(room: Room, participantId: string | undefined) {
  ...
  function emitError(socket: TypedSocket, action: string, err: unknown): void {
  ...
  function startRoomTimerIfNeeded(code: string): void {
  ...
  function startSpeakerTimerIfNeeded(code: string): void {
    startSpeakerTimerLoop(code, (tickFn) => {
      ...
      if (!nextSpeakerTimer) return { finished: true };
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("disconnect", () => {
      ...
      io.to(code).emit("participant:disconnected", { participantId: participantId ?? "" });
      broadcastRoomState(code);
