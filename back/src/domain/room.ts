...
import type { Room, PublicRoom, TimedPhase } from "./types.js";
...
export function resolveAvatarId(value: unknown): string | null {
  return typeof value === "string" && AVATAR_IDS.includes(value) ? value : null;
}

export function resolveStarsPerParticipant(value: unknown, action = "room:create"): number {
  if (value === undefined) return DEFAULT_STARS_PER_PARTICIPANT;
  if (typeof value !== "number" || !Number.isInteger(value) || value < MIN_STARS_PER_PARTICIPANT || value > MAX_STARS_PER_PARTICIPANT) {
...
// los votos ya emitidos (ver domain/cards.ts, toggleVote sigue validando solo
...
export function updateRoomSettings(room: Room, { starsPerParticipant }: { starsPerParticipant?: unknown }): Room {
...
export function resolveSecondsPerSpeaker(value: unknown): number {
  if (value === undefined) return DEFAULT_SECONDS_PER_SPEAKER;
  if (typeof value !== "number" || !Number.isInteger(value) || value < MIN_SECONDS_PER_SPEAKER || value > MAX_SECONDS_PER_SPEAKER) {
    throw new InvalidActionError(
      "room:create",
...
export function resolvePreviousActionNotes(value: unknown): string {
...
export function resolveTeamName(value: unknown): string {
  if (value === undefined || value === null) return "";
  const trimmed = String(value).trim();
...
export function createRoom({
  ...
  return {
    ...
    // Estado efímero de los tildes del Nivel 2 (ver domain/previousAction.ts)
...
export function isRoomLockedForNewJoins(room: Room): boolean {
