...
import type { Phase, TimedPhase, PhaseDurations, Room, TimerState } from "./types.js";
...
export const TIMED_PHASES: TimedPhase[] = [
  "welcome",
  "previous_action",
  "keep_improve_try",
  "grouping_voting",
...
export const DEFAULT_PHASE_DURATIONS_SECONDS: PhaseDurations = {
  welcome: 180,
  previous_action: 300,
...
export function resolvePhaseDurations(partial: Partial<Record<TimedPhase, unknown>> = {}): PhaseDurations {
...
function timerForPhase(phase: Phase, phaseDurations: PhaseDurations): TimerState {
  if (!TIMED_PHASES.includes(phase as TimedPhase)) {
    return { status: "idle", durationSeconds: 0, remainingSeconds: 0 };
  }
  const durationSeconds = phaseDurations[phase as TimedPhase];
  return { status: "running", durationSeconds, remainingSeconds: durationSeconds };
}

export function startSession(room: Room): Room {
...
export function goBackPhase(room: Room): Room {
...
function advanceTo(room: Room, nextPhase: Phase): Room {
  return {
    ...room,
    phase: nextPhase,
